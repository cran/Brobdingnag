...
Just to check, let's call {\tt new()} with arguments of differing
lengths:
...
This is done by multiple calls to {\tt setGeneric()}; see the manpage.
The manpage discusses the {\tt useAsDefault} argument, which will stop
a non-generic function from being used (which in this case is a Bad
Thing).
